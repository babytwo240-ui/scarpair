...
const EditWastePostPage = () => {
  ...
  const fetchPost = async () => {
    try {
      ...
      setFormData({ title: '', description: '', category: '', quantity: '', location: '' });
      setLoading(false);
    } catch (err) {
  ...
  if (loading) return <div style={{ minHeight: '100vh', background: C.darker, color: C.text, padding: 40 }}>Loading post...</div>;
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.darker, color: C.text, padding: '40px' }}>
      ...
      <form onSubmit={handleSubmit} style={{ maxWidth: 600, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 24 }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <label style={{ display: 'block', marginBottom: 8, fontWeight: 700 }}>Update Image</label>
