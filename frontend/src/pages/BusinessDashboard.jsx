...
/* ─── Color tokens (matching LandingPage) ────────────────────────────────────────────
    ...
    <div style={{ minHeight: '100vh', background: C.darker, fontFamily: "'DM Sans','Helvetica Neue',sans-serif", overflowX: 'hidden', color: C.text }}>
        ...
        <div style={{ maxWidth: 1360, margin: '0 auto', padding: '18px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{ fontSize: 14, color: C.textMid }}>
              Welcome, <span style={{ color: C.bright, fontWeight: 700 }}>{user?.businessName || 'Business'}</span>
            </div>
            <button
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                transition: 'all 0.2s',
                boxShadow: '0 0 16px rgba(255,107,107,0.25)',
              }}
              onMouseEnter={e => { e.target.style.boxShadow = '0 0 28px rgba(255,107,107,0.45)'; e.target.style.transform = 'translateY(-1px)'; }}
      ...
      <main style={{ maxWidth: 1360, margin: '0 auto', padding: '80px 40px 60px', position: 'relative', zIndex: 2 }}>
        ...
        <div style={{ marginBottom: 72 }}>
          ...
          <h1 style={{ fontSize: 52, fontWeight: 900, letterSpacing: '-1.8px', color: C.text, margin: 0, lineHeight: 1.1 }}>
            Manage Your<br />Waste Operations
          </h1>
          <p style={{ fontSize: 16, color: C.textMid, maxWidth: 500, marginTop: 20, lineHeight: 1.7 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 18 }}>
          {cardConfig.map((card, i) => (
            <div
              ...
              onMouseEnter={e => {
                e.currentTarget.style.background = 'rgba(100,255,67,0.06)';
            ...
            >
              {/* Icon */}
              <div style={{ width: 56, height: 56, background: 'rgba(100,255,67,0.1)', border: `1px solid ${C.border}`, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, color: C.bright, marginBottom: 24, fontFamily: 'monospace' }}>
                {card.icon}
              </div>

              {/* Content */}
              <h3 style={{ fontSize: 20, fontWeight: 800, color: C.text, margin: '0 0 12px', letterSpacing: '-0.3px' }}>
                {card.title}
              </h3>
              <p style={{ fontSize: 15, color: C.textMid, lineHeight: 1.75, margin: '0 0 24px' }}>
                {card.desc}
              </p>
